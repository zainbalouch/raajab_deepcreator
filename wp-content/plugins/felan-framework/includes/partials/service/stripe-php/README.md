...
```php
\MyStripe\Stripe::setApiKey('sk_test_BQokikJOvBiI2HlWgH4olfQ2');
$customer = \MyStripe\Customer::create([
...
$curl = new \MyStripe\HttpClient\CurlClient();
$curl->setTimeout(10); // default is \MyStripe\HttpClient\CurlClient::DEFAULT_TIMEOUT
$curl->setConnectTimeout(5); // default is \MyStripe\HttpClient\CurlClient::DEFAULT_CONNECT_TIMEOUT

echo $curl->getTimeout(); // 10
echo $curl->getConnectTimeout(); // 5

// tell Stripe to use the tweaked client
\MyStripe\ApiRequestor::setHttpClient($curl);
...
```php
// set up your tweaked Curl client
$curl = new \MyStripe\HttpClient\CurlClient([CURLOPT_PROXY => 'proxy.local:80']);
// tell Stripe to use the tweaked client
\MyStripe\ApiRequestor::setHttpClient($curl);
```
...
```php
\MyStripe\Stripe::setLogger($logger);
```
...
```php
$curl = new \MyStripe\HttpClient\CurlClient([CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1]);
\MyStripe\ApiRequestor::setHttpClient($curl);
```
...
```php
$customers = \MyStripe\Customer::all([],[
    'api_key' => 'sk_test_...',
    'stripe_account' => 'acct_...'
]);

\MyStripe\Customer::retrieve("cus_123456789", [
    'api_key' => 'sk_test_...',
    'stripe_account' => 'acct_...'
]);
```
...
```php
\MyStripe\Stripe::setCABundlePath("path/to/ca/bundle");
```
...
```php
\MyStripe\Stripe::setMaxNetworkRetries(2);
```
...
```php
\MyStripe\Stripe::setEnableTelemetry(false);
```
...
```php
\MyStripe\Stripe::setAppInfo("MyAwesomePlugin", "1.2.34", "https://myawesomeplugin.info");
```
