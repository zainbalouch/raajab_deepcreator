...
## 7.25.0 - 2020-02-14
* [#879](https://github.com/stripe/stripe-php/pull/879) Make `\MyStripe\Collection` implement `\Countable`
...
-   Use `\MyStripe\<class>::class` constant instead of strings ([#643](https://github.com/stripe/stripe-php/pull/643))
...
-   [#539](https://github.com/stripe/stripe-php/pull/539) Fix methods on `\MyStripe\PaymentIntent` to properly pass arguments to the API.

## 6.19.4 - 2018-10-11

-   [#534](https://github.com/stripe/stripe-php/pull/534) Fix PSR-4 autoloading for `\MyStripe\FileUpload` class alias

## 6.19.3 - 2018-10-09

-   [#530](https://github.com/stripe/stripe-php/pull/530) Add constants for `flow` (`FLOW_*`), `status` (`STATUS_*`) and `usage` (`USAGE_*`) on `\MyStripe\Source`
...
-   [#520](https://github.com/stripe/stripe-php/pull/520) Rename `\MyStripe\FileUpload` to `\MyStripe\File`
...
-   [#502](https://github.com/stripe/stripe-php/pull/502) Add `isDeleted()` method to `\MyStripe\StripeObject`

## 6.12.0 - 2018-07-28

-   [#501](https://github.com/stripe/stripe-php/pull/501) Add support for scheduled query runs (`\MyStripe\Sigma\ScheduledQueryRun`) for Sigma
...
-   [#498](https://github.com/stripe/stripe-php/pull/498) Internal improvements to the `\MyStripe\ApiResource.classUrl()` method
...
-   [#493](https://github.com/stripe/stripe-php/pull/493) Add PHPDoc for `auto_advance` on `\MyStripe\Invoice`
...
-   [#481](https://github.com/stripe/stripe-php/pull/481) Add new `\MyStripe\Discount` and `\MyStripe\OrderItem` classes, add more PHPDoc describing object attributes
...
-   [#479](https://github.com/stripe/stripe-php/pull/479) Fix unnecessary traits on `\MyStripe\InvoiceLineItem`

## 6.7.2 - 2018-05-28

-   [#471](https://github.com/stripe/stripe-php/pull/471) Add `OBJECT_NAME` constant to all API resource classes, add `\MyStripe\InvoiceLineItem` class
...
-   [#458](https://github.com/stripe/stripe-php/pull/458) Add PHPDoc for `account` on `\MyStripe\Event`
...
-   [#453](https://github.com/stripe/stripe-php/pull/453) Add constants for `reason` (`REASON_*`) and `status` (`STATUS_*`) on `\MyStripe\Dispute`

## 6.3.2 - 2018-02-27

-   [#452](https://github.com/stripe/stripe-php/pull/452) Add PHPDoc for `amount_paid` and `amount_remaining` on `\MyStripe\Invoice`

## 6.3.1 - 2018-02-26

-   [#443](https://github.com/stripe/stripe-php/pull/443) Add event types as constants to `\MyStripe\Event` class
...
-   [#442](https://github.com/stripe/stripe-php/pull/442) Fix PHPDoc for `\MyStripe\Error\SignatureVerification`
...
*   `\MyStripe\AttachedObject` no longer exists. Attributes that used to be instances of `\MyStripe\AttachedObject` (such as `metadata`) are now instances of `\MyStripe\StripeObject`.

-   Attributes that used to be PHP arrays (such as `legal_entity->additional_owners` on `\MyStripe\Account` instances) are now instances of `\MyStripe\StripeObject`, except when they are empty. `\MyStripe\StripeObject` has array semantics so this should not be an issue unless you are actively checking types.

*   `\MyStripe\Collection` now derives from `\MyStripe\StripeObject` rather than from `\MyStripe\ApiResource`.
...
-   Add `\MyStripe\Stripe::setLogger()` to support an external PSR-3 compatible logger
...
-   Rename `\MyStripe\Object` to `\MyStripe\StripeObject` (PHP 7 compatibility)
...
-   Switched to Stripe namespace instead of MyStripe\_ class name prefiexes (thanks @chadicus!)
