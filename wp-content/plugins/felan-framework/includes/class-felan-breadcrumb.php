    ...
    class Felan_Breadcrumb
    {
        ...
        public static function breadcrumb($args = '')
        {
            ...
            if ($args['type'] == 'string') {
                ...
                $output = apply_filters('felan_breadcrumb_after', $output, $args);
            } else {
                ...
                $output = apply_filters('felan_breadcrumb_before', $output, $args);
                ...
                foreach ($breadcrumb_arr as $ancestor) {
                    ...
                    if ($cnt == count($breadcrumb_arr)) {
                        ...
                        if (class_exists('WooCommerce')) {
                            $output .= '"><a href="' . $ancestor['link'] . '">' . $ancestor['title'] . '</a></li>' . "\n";
                        } else {
                                ...
                                if ($terms) {
                                    ...
                                    if ($term->parent != 0) {
                                        ...
                                        foreach ($ancestors as $ancestor_id) {
                                            ...
                                            if ( ! is_null( $ancestor ) && ! is_wp_error( $ancestor ) && isset( $ancestor->name ) ) {
                                                $breadcrumb_arr[] = array(
                                                    'title' => esc_html( $ancestor->name ),
                                                    'link'  => esc_url( get_term_link( $ancestor, $tax_slug ) )
                                                );
                                            } else {
                                                // Optionally handle the case where $ancestor is null or invalid
                                                error_log('Ancestor is null or contains an error.');
                                            }
                                        }
                                    }
                                    $breadcrumb_arr[] = array(
        ...
        public static function get_month_title($monthnum = 0)
        {
            ...
            if (in_array($date_format, array(
                ...
                preg_match('/(^|[^\\\\]+)(F|m|M|n)/', str_replace('\\\\', '', 'Y-m-d'), $m);
                if (isset($m[2])) {
                    $month_format = $m[2];
                } else {
                    $month_format = 'F';
