...
$comments_query = "SELECT * FROM $wpdb->comments as comment INNER JOIN $wpdb->commentmeta AS meta WHERE comment.comment_post_ID = $freelancer_id AND meta.meta_key = 'freelancer_rating' AND meta.comment_id = comment.comment_ID AND ( comment.comment_approved = 1 OR comment.user_id = $user_id )";
...
$my_review_freelancer_salary_rating = $my_review_freelancer_freelancer_rating  = $my_review_freelancer_skill_rating = $my_review_freelancer_work_rating = '';
$salary_rating_class = $freelancer_rating_class = $skill_rating_class = $work_rating_class = '';
...
if (!is_null($get_comments)) {
    ...
    $freelancer_salary_rating = $freelancer_freelancer_rating = $freelancer_skill_rating = $freelancer_work_rating = array();
    foreach ($get_comments as $comment) {
        if (intval(get_comment_meta($comment->comment_ID, 'freelancer_salary_rating', true)) != 0) {
            $freelancer_salary_rating[]         = intval(get_comment_meta($comment->comment_ID, 'freelancer_salary_rating', true));
        }
        if (intval(get_comment_meta($comment->comment_ID, 'freelancer_freelancer_rating', true)) != 0) {
            $freelancer_freelancer_rating[]         = intval(get_comment_meta($comment->comment_ID, 'freelancer_freelancer_rating', true));
        }
        if (intval(get_comment_meta($comment->comment_ID, 'freelancer_skill_rating', true)) != 0) {
            $freelancer_skill_rating[]         = intval(get_comment_meta($comment->comment_ID, 'freelancer_skill_rating', true));
        }
        if (intval(get_comment_meta($comment->comment_ID, 'freelancer_work_rating', true)) != 0) {
            $freelancer_work_rating[]         = intval(get_comment_meta($comment->comment_ID, 'freelancer_work_rating', true));
        }
        ...
        if (isset($my_review) ? $comment->comment_ID : 0) {
            if ($comment->comment_ID == $my_review->comment_ID) {
                $my_review_freelancer_salary_rating = intval(get_comment_meta($comment->comment_ID, 'freelancer_salary_rating', true));
                $my_review_freelancer_freelancer_rating = intval(get_comment_meta($comment->comment_ID, 'freelancer_freelancer_rating', true));
                $my_review_freelancer_skill_rating = intval(get_comment_meta($comment->comment_ID, 'freelancer_skill_rating', true));
                $my_review_freelancer_work_rating = intval(get_comment_meta($comment->comment_ID, 'freelancer_work_rating', true));
            }
        }
    }
    ...
    if (!empty($freelancer_salary_rating)) {
        $salary_rating = array_sum($freelancer_salary_rating) / count($freelancer_salary_rating);
    ...
    if (!empty($freelancer_freelancer_rating)) {
        $freelancer_rating = array_sum($freelancer_freelancer_rating) / count($freelancer_freelancer_rating);
        $freelancer_rating = number_format((float)$freelancer_rating, 2, '.', '');
        $freelancer_rating_percent = ($freelancer_rating / 5) * 100;
        if ($freelancer_rating_percent >= 0 && $freelancer_rating_percent <= 30) {
            $freelancer_rating_class = 'low';
        } else if ($freelancer_rating_percent >= 31 && $freelancer_rating_percent <= 70) {
            $freelancer_rating_class = 'mid';
        } else if ($freelancer_rating_percent >= 71 && $freelancer_rating_percent <= 100) {
            $freelancer_rating_class = 'high';
        }
    } else {
        $freelancer_rating = 0;
        $freelancer_rating_percent = 0;
    }

    if (!empty($freelancer_skill_rating)) {
        $skill_rating = array_sum($freelancer_skill_rating) / count($freelancer_skill_rating);
    ...
    if (!empty($freelancer_work_rating)) {
        $work_rating = array_sum($freelancer_work_rating) / count($freelancer_skill_rating);
...
?>
<div class="freelancer-review-details block-archive-inner">
        ...
        <div class="rating-bars">
            <div class="rating-bars-item">
                <div class="rating-bars-name">
                    <?php esc_html_e('Clarity in Specification', 'felan-framework'); ?>
                ...
                <span class="rating-bars-inner">
                    <span class="rating-bars-rating <?php echo esc_attr($freelancer_rating_class); ?>" data-rating="<?php echo esc_attr($freelancer_rating); ?>">
                        <span class="rating-bars-rating-inner" style="width: <?php echo esc_attr($freelancer_rating_percent); ?>%;"></span>
                    </span>
                    <span class="value-rating"><?php echo esc_attr($freelancer_rating); ?></span>
                </span>
            </div>
            <div class="rating-bars-item">
                <div class="rating-bars-name">
                    <?php esc_html_e('Communication', 'felan-framework'); ?>
            ...
            <div class="rating-bars-item">
                <div class="rating-bars-name">
                    <?php esc_html_e('Payment Promptness', 'felan-framework'); ?>
            ...
            <div class="rating-bars-item">
                <div class="rating-bars-name">
                    <?php esc_html_e('Professionalism', 'felan-framework'); ?>
    ...
    <div class="duplicate-form-reply hide none">
        ...
        <form method="post" class="repreviewForm" action="#">
            ...
            <button type="submit" class="felan-submit-freelancer-reply felan-button"><?php esc_html_e('Send', 'felan-framework'); ?></button>
            <?php wp_nonce_field('felan_submit_reply_ajax_nonce', 'felan_security_submit_reply'); ?>
            <input type="hidden" name="action" value="felan_freelancer_submit_reply_ajax">
